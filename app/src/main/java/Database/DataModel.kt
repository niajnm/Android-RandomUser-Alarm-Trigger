package Database

class DataModel {


    var historyID: String? = null
    var historyName: String? = null
    var historyAge: String? = null
    var historyGender: String? = null
    var historyCountry: String? = null
    var historyPhone: String? = null
    var historyMail: String? = null
    var historyImg: String? = null
    var historyDeleteId: String? = null

    var alarmTime: String? = null
    var alarmTime2: String? = null
    var alarmTMilsec: Long? = null
    var alarmTMilsec2: Long? = null
    var medicineName: String? = null
    var medicineName2: String?=null
    var alarmDays: String? = null
    var requestCode: Int? = null
    var requestCode2: Int? = null
    var alarmMagicKey : Int? = null


    var bpId:Int?=null
    var bpName:String?=null
    var bpDate:String?=null
    var bpTime:String?=null
    var bpSys:String?=null
    var bpDias:String?=null
    var bpPulse:Int?=null
    var bpResult:String?=null
    var bpMap:Int?=null
    var bpColor:Int?=null
    var bpChart:Int?=null
    var bpPosition:String?=null
    var bpExtrimity:String?=null




}